...
if [ ! -f "$output_file" ]; then
    echo "Location,Videos (B), Pictures (B),Music (B),Downloads (B),Documents (B),Desktop (B),Total GB" > "$output_file"
fi

# Calculate the total size in GB
total=0
total=$(echo "scale=2; ${video_totals}+${pics_totals}+${music_totals}+${down_totals}+${docs_totals}+${desk_totals}" | bc)
total=$(echo "scale=2; ${total}/(1024*1024*1024)" | bc)

# Output the result to a CSV file
echo "$location,$video_totals,$pics_totals,$music_totals,$down_totals,$docs_totals,$desk_totals,$total" >> "$output_file"
